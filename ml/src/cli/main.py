...
def show_config():
    ...
    try:
        ...
        for task_name, task_config in config.features.items():
            ...
            feature_count = len(
                task_config.numeric_features + task_config.categorical_features
            )
...
def print_training_summary(results: Dict[str, Any]):
    ...
    print(
        ...
        f"{results['data_info']['test_samples']} test"
    )
...
Examples:
  ...
  python -m src.cli.main config validate
    ...
    train_parser.add_argument(
        "--optimize", action="store_true", help="Optimize hyperparameters"
    )
