...
class DataValidator:
    ...
    def __init__(self, expectations_dir: Optional[str] = None):
        ...
        self.expectations_dir = (
            Path(expectations_dir) if expectations_dir else Path("expectations")
        )
        ...
        logger.info(
            "Data validator initialized with expectations directory: "
            f"{self.expectations_dir}"
        )
                ...
                stores={
                    "expectations_store": {
                        ...
                        "store_backend": {
                            ...
                            "base_directory": str(
                                self.expectations_dir / "expectations"
                            ),
                        },
                    },
                    "validations_store": {
                        ...
                        "store_backend": {
                            "class_name": "TupleFilesystemStoreBackend",
                            "base_directory": str(
                                self.expectations_dir / "validations"
                            ),
                        },
                    },
    ...
    def create_expectation_suite(
        self, suite_name: str, data_type: str = "openfda_events"
    ) -> ExpectationSuite:
        """
    ...
    def _add_openfda_expectations(self, suite: ExpectationSuite) -> None:
        ...
        suite.add_expectation(ge.expectations.ExpectColumnToExist("patientonsetage"))
        suite.add_expectation(ge.expectations.ExpectColumnToExist("patientsex"))
        suite.add_expectation(ge.expectations.ExpectColumnToExist("reaction"))
        suite.add_expectation(ge.expectations.ExpectColumnToExist("brand_name"))
        suite.add_expectation(ge.expectations.ExpectColumnToExist("serious"))
        ...
        suite.add_expectation(
            ge.expectations.ExpectColumnValuesToBeInSet(
                column="patientsex", value_set=["M", "F", "U", "Unknown"]
            )
        )
        ...
        suite.add_expectation(
            ge.expectations.ExpectColumnValuesToBeInSet(
                column="serious", value_set=[0, 1, 2]
            )
        )
        ...
        suite.add_expectation(
            ge.expectations.ExpectColumnValuesToNotBeNull(
                column="serious", mostly=0.8  # Allow 20% missing
            )
        )
    ...
    def _add_synthea_expectations(self, suite: ExpectationSuite) -> None:
        ...
        suite.add_expectation(ge.expectations.ExpectColumnToExist("patient_id"))
        suite.add_expectation(ge.expectations.ExpectColumnToExist("encounter_id"))
        suite.add_expectation(ge.expectations.ExpectColumnToExist("condition_code"))
    ...
    def validate_data(
        self, df: pd.DataFrame, suite_name: str, data_asset_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _process_validation_results(self, validation_result) -> Dict[str, Any]:
        ...
        results = {
            ...
            "total_expectations": validation_result.statistics[
                "evaluated_expectations"
            ],
            "successful_expectations": validation_result.statistics[
                "successful_expectations"
            ],
            "failed_expectations": validation_result.statistics[
    ...
    def validate_training_data(
        self, df: pd.DataFrame, target_column: str, data_type: str = "openfda_events"
    ) -> Dict[str, Any]:
        """
    ...
    def _add_ml_expectations(self, suite: ExpectationSuite, target_column: str) -> None:
        ...
        suite.add_expectation(ge.expectations.ExpectColumnToExist(target_column))

        suite.add_expectation(
            ge.expectations.ExpectColumnValuesToNotBeNull(
                column=target_column, mostly=0.9  # Allow 10% missing targets
            )
        )
        ...
        suite.add_expectation(
            ge.expectations.ExpectTableRowCountToBeBetween(
                min_value=1000, max_value=10000000  # Minimum samples for training
            )
        )

    def generate_data_quality_report(
        self, df: pd.DataFrame, suite_name: str, output_path: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            if output_path is None:
                output_path = (
                    self.expectations_dir / f"data_quality_report_{suite_name}.json"
                )
