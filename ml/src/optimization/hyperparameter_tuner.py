...
class HyperparameterTuner:
    ...
    def optimize_classification(
        ...
        self.study = optuna.create_study(
            direction=self.direction, study_name=study_name
        )
    ...
    def optimize_with_mlflow(
        ...
        """
        logger.info(
            f"Starting MLflow-integrated optimization for {model_class.__name__}"
        )

        # Create MLflow callback
        mlflow_callback = optuna.integration.mlflow.MLflowCallback(
            tracking_uri=experiment_tracker.config.tracking_uri, metric_name=scoring
        )
