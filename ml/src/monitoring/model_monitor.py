...
class ModelMonitor:
    ...
    def __init__(self, models_dir: Optional[str] = None):
        ...
        logger.info(
            f"Model monitor initialized with models directory: {self.models_dir}"
        )
    ...
    def set_baseline(
        ...
        if predictions_baseline is not None:
            baseline_stats["predictions_stats"] = self._calculate_prediction_statistics(
        ...
        for column in X.columns:
            if X[column].dtype in ["int64", "float64"]:
                ...
                stats_dict[column] = {
                    ...
                    "most_frequent_ratio": (
                        float(value_counts.iloc[0] / len(X))
    ...
    def detect_data_drift(
        self, model_name: str, X_current: pd.DataFrame, drift_type: str = "all"
    ) -> Dict[str, Any]:
        """
        ...
        for column in X_current.columns:
            if column in baseline_stats["feature_stats"]:
                feature_drift = self._detect_feature_drift(
                    baseline_stats["feature_stats"][column], X_current[column], column
                )
        ...
        drift_scores = [
            result["drift_score"]
            for result in drift_results["feature_drift"].values()
        ...
        if drift_scores:
            ...
            drift_results["overall_drift"] = (
                drift_results["drift_score"] > self.drift_thresholds["psi_threshold"]
            )
    ...
    def _detect_feature_drift(
        self, baseline_stats: Dict[str, Any], current_data: pd.Series, feature_name: str
    ) -> Dict[str, Any]:
        ...
        if "mean" in baseline_stats:  # Numeric feature
            drift_result.update(
                self._detect_numeric_drift(baseline_stats, current_data)
            )
        else:  # Categorical feature
            drift_result.update(
                self._detect_categorical_drift(baseline_stats, current_data)
            )

        return drift_result

    def _detect_numeric_drift(
        self, baseline_stats: Dict[str, Any], current_data: pd.Series
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            std_drift = (
                abs(current_std - baseline_std) / baseline_std
                if baseline_std > 0
                else 0
            )
    ...
    def _calculate_psi(
        ...
        for value in all_values:
            ...
            if baseline_ratio > 0 and current_ratio > 0:
                psi += (current_ratio - baseline_ratio) * np.log(
    ...
    def detect_performance_drift(
        ...
        if len(self.monitoring_data[model_name]) > 1:
            ...
            for metric_name, current_value in current_metrics.items():
                if metric_name in historical_metrics:
                    ...
                    if (
                        performance_drop
                        > self.drift_thresholds["accuracy_drop_threshold"]
                    ):
        ...
        logger.info(
            ...
            f"Drift detected: {performance_drift['performance_drift_detected']}"
        )
    ...
    def _get_historical_metrics(
        ...
        historical_data = self.monitoring_data[model_name][
            -window_size:-1
        ]  # Exclude current
    ...
    def generate_monitoring_report(
        ...
        if output_path is None:
            ...
            output_path = f"monitoring_report_{model_name}_{timestamp}.json"
        ...
        if model_name in self.monitoring_data and self.monitoring_data[model_name]:
            ...
            if len(self.monitoring_data[model_name]) >= 3:
                recent_accuracy = [
                    ...
                    for point in self.monitoring_data[model_name][-3:]
                ]
                if all(
                    recent_accuracy[i] >= recent_accuracy[i - 1]
                    for i in range(1, len(recent_accuracy))
                ):
    ...
    def plot_drift_analysis(
        ...
        if output_path is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = f"drift_analysis_{model_name}_{timestamp}.png"
        ...
        drift_scores = [
            drift_results["feature_drift"][f]["drift_score"] for f in features
        ]

        axes[0, 0].bar(features, drift_scores)
        axes[0, 0].axhline(
            y=self.drift_thresholds["psi_threshold"],
            color="r",
            linestyle="--",
            label="Threshold",
        )
        ...
        drift_detected = [
            drift_results["feature_drift"][f]["drift_detected"] for f in features
        ]
        drift_counts = {
            "No Drift": drift_detected.count(False),
            "Drift Detected": drift_detected.count(True),
        }

        axes[0, 1].pie(
            drift_counts.values(), labels=drift_counts.keys(), autopct="%1.1f%%"
        )
        ...
        if model_name in self.monitoring_data and self.monitoring_data[model_name]:
            timestamps = [
                ...
                for point in self.monitoring_data[model_name]
            ]
            accuracies = [
                point["metrics"]["accuracy"]
                for point in self.monitoring_data[model_name]
            ...
            axes[1, 0].text(
                0.5, 0.5, "No performance data available", ha="center", va="center"
            )
            axes[1, 0].set_title("Model Performance Over Time")
