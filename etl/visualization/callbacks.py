...
def update_visuals(dataset, start_date, end_date):
    ...
    main_chart = px.line(
        df, x="date", y="count", title=f"{dataset.capitalize()} Records Over Time"
    )

    category_chart = px.bar(
        df_cat, x="category", y="count", title=f"{dataset.capitalize()} by Category"
    )
    ...
    trend_chart.add_trace(
        go.Scatter(x=df_trend["date"], y=df_trend["count"], mode="lines", name="Daily")
    )
    trend_chart.add_trace(
        go.Scatter(
            x=df_trend["date"],
